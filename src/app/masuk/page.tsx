  ...
  const handleEmail = useCallback(
    async (e: React.FormEvent) => {
      ...
      if (error) {
        ...
        setMessage({
          tone: "success",
          text: `Kode/tautan masuk sudah kami kirim ke ${email}. Periksa inbox (dan folder spam).`,
        });
      }
      setLoading(null);
    },
    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="w-full max-w-sm">
        <div className="mb-6 flex flex-col items-center gap-2 text-center">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Masuk ke Faza Studio</h1>
          <p className="text-sm text-muted-foreground">Lanjutkan membuat konten viral.</p>
        </div>

        <Card>
          <CardHeader>
            <CardTitle className="text-base">Masuk</CardTitle>
            <CardDescription>Pilih cara masuk favoritmu.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <form onSubmit={handleEmail} className="space-y-3">
              ...
              <Button
                ...
                disabled={!!loading}
              >
                {loading === "email" ? "Mengirim…" : "Masuk dengan Email"}
              </Button>
            </form>
            ...
            <p className="text-center text-sm text-muted-foreground">
              Belum punya akun?{" "}
              <Link href={`/daftar?next=${encodeURIComponent(next)}`} className="font-medium text-primary underline underline-offset-2">
                Daftar
              </Link>
            </p>
          </CardContent>
